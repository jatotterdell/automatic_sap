...
\begin{versionhistory}
      \vhEntry{0.1}{2020-01-17}{JT}{Draft created.}
      \vhEntry{0.2}{2020-03-17}{JT, JM}{Incorporate estimands, re-work secondary analysis outline.}
      \vhEntry{0.3}{2020-09-18}{JT}{Update plan to allow dropping of control group and simplify decision rules.}
      \vhEntry{0.4}{2020-10-01}{JT}{Finalise simulation results.}
  \end{versionhistory}
...
\sigma_\xi^2 &\sim \text{Half-}t(3, 0, 2.5) \\
\tau_e|\sigma_\tau^2 &\sim N(\tau_{e-1}, \sigma_\tau^2) \\
\sigma_\tau^2 &\sim \text{Half-}t(3, 0, 2.5) \\
\tau_1 &= 0.
\end{aligned}
\]
...
(2)\quad s_{jk} &= \gamma_{0j(t)} + \gamma_{1j(t)}k + \textstyle\sum_{m=1}^M g_{j(t)m} B_{mk} \\
\gamma &\sim N(0,2.5^2) \\
g_{jm}|\omega_j^2 &\sim N(0, \omega_j^2) \\
\omega_j &\sim \text{Half-}t(3, 0, 10).
\end{aligned}
\]
where \(j(t)\) returns the timing of intervention \(j\).
That is \((1)\) represents combination specific curves and \((2)\) timing specific curves averaged across message framings.
