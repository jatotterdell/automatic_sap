...
def <- data.frame(
  ...
  "Definition" = c(
    "Estimands align protocol objectives with the quantification of the intervention effect",
...
**Objective:** To determine the real-world comparative effectiveness of the first vaccine-provider initiated SMS reminder content and timing for increasing the proportion of children vaccinated within 28 days of scheduled routine vaccine date.
...
Therefore, for records where it is not possible to determine beyond a reasonable doubt which scheduled childhood vaccine is being administered, the parent will be excluded from the statistical analysis.  
...
**Objective:** To determine the real-world timeliness of first vaccine-provider initiated SMS reminder content and timing for reducing the time to vaccination for routine childhood vaccines.
...
**Endpoint:** Time to vaccination calculated as date of vaccine administration minus date of randomisation (28 days before date of scheduled vaccination).
Vaccine administration which occurs later than 90 days after randomisation or within 28 days of a subsequently scheduled vaccination 
...
Each child may receive more than one scheduled vaccination during the study period, but all parents will receive the same intervention for all eligible children under their care for all scheduled vaccinations.
...
This is the run-in period before response-adaptive randomisation commences.
...
In presenting the relevant details, refer to Table \@ref(tab:intsum) for the breakdown of intervention arm groupings according to message framing and timings as used in this document.
...
The prior distributions for the intercept and average intervention effect parameters are set as non-informative
$$
...
s_0(t) &= \beta_0 + \beta_1 + \sum_{l=1}^L b_l z_l(t),\ b|\sigma^2, \ \sigma^2\sim \text{IG}(a_\sigma,b_\sigma) \\
s_j(t) &= \gamma_{0j} + \gamma_{1j} + \sum_l^L g_{jl} w_l(t),\ g_j | \tau_j^2, \ \tau_j^2\sim\text{IG}(a_\tau,b_\tau)
\end{aligned}
$$
where $z_l$ are spline bases enforcing a smoothing penalty and $s_j$ is the curve for individuals $i$ in group $j$ for the chosen link function $h$.
...
If issues occur with the above model we will also consider the simplified grouped proportional hazards model
$$
\begin{aligned}
...
\end{aligned}
$$
where the baseline hazard is modelled as previously, but each intervention effect is assumed to an additive constant on the transformed hazard scale.
...
The effect of SMS vaccine reminders on the primary endpoint will be determined in the final analysis for the following subgroups: 
...
The chosen design controlled the family-wise false positive rate at 0.05 and provided at least 70% power when one framing and timing combination was superior by at least 0.25 in the log-odds of 28 day vaccination compared to all other inteventions with log-odds of response at 1.0.
