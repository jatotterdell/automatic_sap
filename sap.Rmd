...
def <- data.frame(
  ...
  "Definition" = c(
    "The first child scheduled for vaccination for a given parent after the parent has been randomised.",
    "The first scheduled vaccine date for the index child after the parent’s randomisation date.",
    "A child who has not received the recommended vaccinations within 4 weeks (28 days) of the scheduled date according to the standard childhood immunisation schedule.",
    "Participating SmartVax GPs and community-based providers."
  )
)
ktab <- knitr::kable(def, booktabs = TRUE, linesep = "") 
...
In addition, the optimal content of the messages will be examined, as a neutral SMS reminder may be all that is necessary to convince parents to vaccinate on time. 
...
Following vaccine administration, the SmartVax platform will be able to use the details of the vaccination (entered electronically by site staff), to schedule the next SMS reminder. 
...
Twelve intervention arms, consisting of the combination of four different message framings and three different timings will be investigated.
...
To determine the effect of different timing and content of a personalised SMS reminder, emanating from a family’s vaccine provider (general practice or community vaccination clinic), on the proportion of children vaccinated within 28 days of the scheduled due date for routine childhood vaccination compared to usual practice. 
...
In addition to the response adpative randomisation, the trial also allows early stopping for:
...
The primary endpoint is vaccination status at 28 days after the vaccine measured by the difference between date of vaccine administration and due date as recorded in the SmartVax system for the *index vaccination* of each parent randomised.
Refer to Section \@ref(definitions) for definitions.

The parent (whose behaviour we seek to change) can only be considered naïve to the intervention on the first child-vaccination occasion, therefore, the primary analysis will be performed on the outcome of the first intervention occasion for each parent. 
...
The parent is the effective unit of randomisation, as it is their behaviour we seek to change.
Parents of eligible children will therefore be ranodmised to one of the 13 arms.
Each child may receive more than one scheduled vaccination during the study period; all children of the parent will receive the same intervention for all of their scheduled vaccinations.
...
Prior to the first interim analysis, parents will be equally likely to be randomised to any of the intervention or control arms.
Following subsequent interim analyses, the allocation ratios will be updated and a new allocation list generated on the middleware application. 
Newly randomised parents will then be allocated to arms according to the new allocation list.
...
Study participation (parent and child/children) is from parent’s randomisation until the last scheduled vaccine dose has been administered after four years of age for any child under their care or until the trial concludes.

## Eligibility criteria

To be included in the study ALL of the following criteria must be satisfied;

* Parents of children aged 6 weeks – 4 years who are registered with a SmartVax registered GP clinic or community vaccination clinic or whose clinic has expressed interest in SmartVax.
...
The primary analysis dataset consists of index vaccines.
The secondary outcome datasets will consist of all children and all vaccines.

The data will be analysed and reported on an intention-to-treat (ITT) basis with all randomised participants contributing.
In particular, we cannot know that the SMS reminders were recieved or read by the participants, only that they were sent.
...
Individual-level de-identified and encrypted data for every enrolled child will be exported electronically and in an encrypted fashion to the secure REDCap database on the Telethon Kids Institute server. 
...
In presenting the relevant details, refer to Table \@ref(tab:intsum) for the breakdown of intervention arm groupings according to message content and timings as used in this document.

In what follows, the message contents are denoted by $a$ and message timings by $b$. 
...
The prior distributions for the intercept and average intervention effect parameters are set as weakly informative
$$
\alpha_0,\alpha_1\sim N(0, 10).
$$
The priors on the $\gamma$ terms are set so as to enforce the relevant sum-to-zero constraints. 
We would expect interaction effects to be small relative to the main effects, which are reflected by the chosen prior co-variances.
$$
\begin{aligned}
S_l &= I_l - l^{-1}J_l \\
\gamma^a &\sim N(0, 10S_4) \\
\gamma^b &\sim N(0, 10S_3) \\
\gamma^{ba} &\sim N(0, S_4\otimes S_3).
\end{aligned}
$$
The prior on other model parameters will be weakly informative
$$
\beta \sim N(0, 10).
$$
...
\end{aligned}
$$
The parameter $\tau$ has a first-order random walk prior to smooth baseline changes in the response across cohorts. 
The prior on $\sigma_\tau^2$ highlights that we expect small changes in baseline rates between cohorts, but that the posterior will be highly influenced by the obeserved data.
This specification does not take into account the time between recruitment of cohorts. 
...
Denote the number of draws by $M$. For each of the 13 arms, define $\mu_j$ as the log-odds of vaccination by 28 days for arm $j=0,...,12$ and define $\theta_{jk} = \mu_j - \mu_k$ as the pairwise differences for $j,k=0,...,12$.
...
It may be that many or most intervention arms appear similarly effective, in which case, they could all have approximately equal posterior probablity of being superior.
...
Interim analyses are scheduled to occur every 500 participants who meet the primary endpoint. 
At each analysis there will be index cases who are enrolled but yet to reach 28 days after their due vaccination; these will be excluded from the analysis.
...
If numbers allow, the effect of SMS vaccine reminders on the primary endpoint will be determined in the final analysis for the following subgroups: 
...
Secondary outcome 1. will be analysed analagously to the primary outcome. 
...
We specify a semiparametric hazard regression model and will allow for group specific curves, $j=1,...,12$ to deviate from the baseline curve of no intervention, $j=0$, with the curves modelled by P-splines [@eilers1996; @currie2002]
$$
\begin{aligned}
\eta_{ijt} &= s_0(t) + s_{j(i)}(t) \\
\lambda_{ijt} &= h(\eta_{ijt}) \\
s_0(t) &= \beta_0 + \beta_1 + \sum_{l=1}^L b_l z_l(t),\ b|\sigma^2\sim RW(1,\sigma^2),\ \sigma^2\sim \text{IG}(a_\sigma,b_\sigma) \\
s_j(t) &= \gamma_{0j} + \gamma_{1j} + \sum_k^K g_{jk} w_k(t),\ g_j | \tau_j^2 \sim RW(1,\tau^2_j),\ \tau_j^2\sim\text{IG}(a_\tau,b_\tau)
\end{aligned}
$$
where $z_l$ and $w_l$ are suitable spline bases and $s_j$ is the curve for individuals $i$ in group $j$ for the chosen link function $h$.
...
For this model we focus on the restricted mean survival time (RMST) [@royston2011; @royston2013] to $t^\star$ in each group as the paramter of interest, defined as
$$
\text{RMST}_j(t^\star) = \int_0^{t^\star} S_j(t) dt
$$
with $S_j(t)$ the survival curve obtained from the above model for group $j$. 
In particular, we will compare groups according to the difference in their RMST.

If issues occur with the above model we will also conider the simplified grouped proportional hazards model
$$
\begin{aligned}
\eta_{it} &= \gamma_t + x_{it}^\top\beta \\
\lambda_{it} &= h(\eta_{it})
\end{aligned}
$$

The variables $x_{it}$ are time-varying indicators for the intervention received (message and timing).
The relevant components of $x_{it}$ only take on the value 1 on the day the SMS was sent, prior to which all $x_{it}$ elements are equal to 0. The $\gamma_t$ parameters will be smoothed across time points by modelling them as a P-spline term on time, $\gamma_t = f(t)$.

Due to nesting of vaccinations within children, and children within parents, secondary outcome 3. will include additional random effects terms in the linear predictor to account for the grouping of children and their vaccinations.
...
The chosen design controlled the family-wise false positive rate at 0.05 and provided at least 70% power when one content and timing combination was superior by at least 0.25 in the log-odds of 28 day vaccination compared to all other inteventions with log-odds of response at 1.0.
